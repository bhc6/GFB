...
annotation_data_path = os.path.join(
    os.path.dirname(__file__),
    'automatic_prompt_engineer/experiments/data/instruction_induction/annotations/'
)
...
def load_ii_data(task):
    ...
    if len(train_inputs) > 32:
        train_ds = Dataset.from_dict({
            'text': train_inputs[:32],
            'label': train_outputs[:32]
        })
        validation_ds = Dataset.from_dict({
            'text': train_inputs[64:96],
            'label': train_outputs[64:96]
        })
    else:
        train_ds = Dataset.from_dict({
            'text': train_inputs,
...
def get_exact_set_score(prediction, ground_truth):
    prediction_normalized = normalize_prediction(prediction,
                                                 lowercase=True).split()
    ground_truth_normalized = normalize_prediction(ground_truth,
                                                   lowercase=True).split()
...
def get_contains_score(prediction, ground_truth):
    ...
    ground_truth_normalized = normalize_prediction(ground_truth,
                                                   lowercase=True)
    if re.search(r'\b({0})\b'.format(ground_truth_normalized),
...
def ii_tta_evaluation(
    ...
    for batch in tqdm(dataloader):
        ...
        generated_texts = _get_generated_text(meta_prompted_inputs,
                                              generated_texts)
        ...
        with torch.no_grad():
            outputs = target_model.generate(prompted_input_ids,
                                            **generation_kwargs)
        generated_texts_ = target_tokenizer.batch_decode(
            outputs, skip_special_tokens=True)
        generated_texts = _get_generated_text(prompted_inputs,
                                              generated_texts_)
...
def ii_tta_evaluation_test(
    ...
    for batch in tqdm(dataloader):
        inputs = batch['text']
        labels = batch['label']
        prompted_inputs = _format_prompts(
            [meta_prompt for i in range(len(inputs))], inputs)
    ...
    with torch.no_grad():
        for prompt in prompts:
            ...
            for batch in dataloader:
                ...
                prediction_ = tokenizer.decode(outputs[0],
