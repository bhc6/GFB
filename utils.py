...
class TopAccuracyTextsScore:
        ...
        if text in self.only_text:
            ...
            if len(self.heap) < self.max_size:
                heapq.heappush(self.heap,
                               (accuracy, len(text), text, ep, score))
    ...
    def get_top_texts(self):
        return sorted([(accuracy, text, ep, score)
                       for accuracy, _, text, ep, score in self.heap],
                      reverse=True)


# Get reward function
def evaluation_full(prompts,
                    imdb,
                    model,
                    tokenizer,
                    device,
    ...
    for prompt in prompts:
        ...
        yes_token_id = tokenizer.encode(verbalizer[0],
                                        add_special_tokens=False)[0]
        no_token_id = tokenizer.encode(verbalizer[1],
                                       add_special_tokens=False)[0]
...
def language_feedback(
    ...
    feedback_query = [{
        ...
        "user",
        "content":
        """This is output instruction and it's accuracy. Please describe the problems of the instruction generated to solve this problem. \nTasks : """
        + args.meta_question + '\n' + example + feedback
    }, {
        "role": "assistant",
        "content": " Problems : "
    }]
    feedback_output = ppo_trainer.generate(ppo_tokenizer.apply_chat_template(
        feedback_query, return_tensors='pt').view(-1).to(device),
                                           **generation_kwargs,
                                           return_prompt=False,
                                           num_return_sequences=1)
    feedback_output = ppo_tokenizer.decode(feedback_output.squeeze(),
                                           skip_special_tokens=True)
    update_query = [{
        "role":
        "user",
        "content":
        'Please write instructions to fix this problem.\n' + feedback_output
    }, {
        "role": "assistant",
        "content": "Instruction : "
    }]
    update_output = ppo_trainer.generate(ppo_tokenizer.apply_chat_template(
        update_query, return_tensors='pt').view(-1).to(device),
                                         **generation_kwargs,
                                         return_prompt=False,
                                         num_return_sequences=1)
    update_output = ppo_tokenizer.decode(update_output.squeeze(),
                                         skip_special_tokens=True)
    updated_query_text = [{
        "role":
        "user",
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                formatted_templates = _format_prompts(current_prompts,
                                                      inputs,
                                                      side=side)
                all_logits = _get_logits(formatted_templates, tokenizer, model,
                                         device)
...
def create_balanced_subset(dataset, subset_size, label_key='label'):
    ...
    while len(subset) < subset_size:
        for label, items in by_label.items():
            if len(subset) < subset_size and len(
                    items) > len(subset) // len(by_label):
...
def create_balanced_subset_and_validation(dataset,
                                          subset_size,
                                          label_key='label'):
    # Group dataset by label
    by_label = {}
    ...
    for label, items in by_label.items():
        ...
        validation_set.extend(
            validation_items[:min(per_class, len(validation_items))])
...
def evaluation_soft_apo(prompts,
                        inputs,
                        targets,
                        model,
                        tokenizer,
    ...
    verbalizer_ids = tokenizer.convert_tokens_to_ids(verbalizer)
    for prompt in prompts:
        formatted_templates = _format_prompts([prompt] * len(inputs),
                                              inputs,
                                              side=side)
...
def evaluation_cot(
    ...
    def _get_answer(texts, tokenizer, model, device):
        encoded_texts = tokenizer.apply_chat_template(
            texts, return_tensors='pt').to(device)
        generated_output = model.generate(encoded_texts, **generation_kwargs)
        generated_output = tokenizer.decode(generated_output.squeeze(),
                                            skip_special_tokens=True)
    ...
    for prompt in prompts:
        ...
        middel_answer = _get_answer(chat_template_input, tokenizer, model,
                                    device)
...
def evaluation_generation(prompts,
                          dataset,
                          model,
                          tokenizer,
                          device,
        ...
        if show:
            for prompt in prompts:
                ...
                for batch in tqdm(dataloader):
                    ...
                    prompt_encoded = tokenizer(template,
                                               return_tensors='pt').to(device)
                    outputs = model.generate(**prompt_encoded,
                                             max_new_tokens=10,
                                             do_sample=True)
                    e_outputs = tokenizer.decode(outputs[0],
                                                 skip_special_tokens=True)
                    r_outputs = tokenizer.decode(
                        outputs[0][len(prompt_encoded[0]):],
...
def _format_prompt(prompt, inputs):
    template = "{prompt} Input : {sentence_1} Output : "
    return [
        template.format(prompt=prompt, sentence_1=inputss)
...
def tta_evaluation(
    ...
    for batch in tqdm(dataloader):
        ...
        generated_texts = _get_only_generated_tta(meta_prompted_inputs,
                                                  generated_texts)
        prompted_inputs = _format_prompts(generated_texts, inputs)
        all_logits = _get_logits_tta(prompted_inputs, target_tokenizer,
                                     target_model, device)
        verbalizer_logits = all_logits[:, verbalizer_ids]
        log_probs = F.softmax(verbalizer_logits, dim=1)
        preds = torch.argmax(log_probs, dim=1).cpu()
...
def tta_evaluation_(
    ...
    for batch in tqdm(test_dataloader):
        with torch.no_grad():
            inputs = batch['text']
            labels = batch['label']
            examples = got_example(validation_dataset,
                                   verbalizer,
            ...
            query_encoded = agent_tokenizer.apply_chat_template(
                query_text, return_tensors='pt').view(-1).to('cuda:0')
            response_tensors = ppo_trainer.generate(query_encoded,
                                                    **generation_kwargs,
                                                    return_prompt=False,
                                                    num_return_sequences=1)
            used_prompt = [
...
def get_em_score(prediction, ground_truth):
    ...
    ground_truth_normalized = normalize_prediction(ground_truth,
                                                   lowercase=True)
...
def normalize_prediction(prediction, lowercase=True):
    ...
    prediction = prediction.translate(str.maketrans('', '',
                                                    string.punctuation))
...
def get_f1_score(prediction, ground_truth):
    prediction_tokens = normalize_prediction(prediction,
                                             lowercase=True).split()
    ground_truth_tokens = normalize_prediction(ground_truth,
                                               lowercase=True).split()
...
def tta_evaluation_bbh(
    ...
    for batch in tqdm(dataloader):
        ...
        generated_texts = target_tokenizer.batch_decode(
            outputs, skip_special_tokens=True)
        generated_texts = _get_only_generated(generated_texts,
                                              'Instruction : ')
        prompted_inputs = _format_prompts(generated_texts, inputs)
        if metrics == 'multiple_choice_grade':
            all_logits = _get_logits_tta(prompted_inputs, target_tokenizer,
