...
def main():
    ...
    for ep in tqdm(range(args.epochs)):
        ...
        wandb.log(
            {
                'batch/accuracy_mean': float(torch.mean(accuracies_tensor)),
    ...
    if final_prompts:
        ...
        for i, (score, text, ep) in enumerate(prompt_queue):
            print('[prompt] :', text, '[score] :', new_acc[i], '[epoch] :', ep)
        ...
        final_results_table = wandb.Table(
            columns=['Rank', 'Prompt', 'Score', 'Epoch'],
            data=[[
        ...
        print('No prompts in top-k queue; skipping final evaluation.')
