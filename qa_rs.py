...
def main():
    ...
    for ep in tqdm(range(args.epochs)):
        ...
        for r in response_tensors:
            full_response = agent_tokenizer.decode(r.squeeze(), skip_special_tokens=True)
            if "The Instruction is :" in full_response:
                instruction = full_response.split("The Instruction is :")[-1].strip()
            else:
